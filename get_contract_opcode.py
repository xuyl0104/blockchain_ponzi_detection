            ...
            for row in csv_reader:
                if 0 < line_count:
                    ...
                    if opcode is not None:
                        file_name_to_save = './data/contracts/ponzi/' + row[1] + '.txt'
        ...
        with open(self.non_ponzi_filename, 'r') as non_ponzi_file:
            ...
            for row in csv_reader:
                if 0 < line_count:
                    ...
                    opcode = self.search_opcode_by_contract_address(row[1])
                    if opcode is not None:
                        file_name_to_save = './data/contracts/nonponzi/' + row[1] + '.txt'
                        if not os.path.exists(file_name_to_save):
                            with open(file_name_to_save, 'w+') as f:
                                f.write(" ")
                        with open(file_name_to_save, 'w+') as write_file:
...
if __name__ == '__main__':
    opcode_collector = OpcodeColector('./ponziContracts.csv', './non_ponziContracts.csv', './Opcodes.csv')
